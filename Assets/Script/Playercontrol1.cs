...
public class Playercontrol1 : MonoBehaviour, IDamageable
{
    ...
    public void RotateToInputVector()
    {
    ...
    {
        if (!GameManager.Instance.IsControlling) { inputVector = Vector3.zero; }
    ...
    void SwitchPlayerStates()
    {
        ...
        if (inputVector.magnitude == 0|| !GameManager.Instance.IsControlling)
        {
    ...
    void Camcontrol()
    {
        ...
        if (currentTarget != null)
        {
            ...
            cameraPosition = transform.position + direction * lockondistance + Vector3.up * 1.5f - right * 1.5f;
    ...
    public void Switch()
    {
        if(MissionManager.Instance.GetNowMission().type == MissionType.switchCharacter)
        {
            MissionManager.Instance.AccomplishMission();
        }
        if (!GameManager.Instance.IsthisCtrolingCharacter(thisCharacter))
        {
    ...
    public void TakeDamage(int damage, float knockback, Transform attackform, ShakeStrength shakeStrength= ShakeStrength.LIGHT)
    {
        ...
        if (GameManager.Instance.IsthisCtrolingCharacter(thisCharacter))
        { CinemachineShakeManager.Instance.Shake(shakeStrength); }

        GameManager.Instance.PlayerHitStop(thisCharacter, GameManager.Instance.hitstopduration);
    ...
    void IDamageable.TakeDamage(int damage, int stun, float knockback, AllPlayerCharacter attacker, ShakeStrength shakeStrength )
    {
        return;
    }
