...
public class AttackGenerator : MonoBehaviour
{
    ...
    public CinemachineShakeManager.ShakeStrength currentShakeStrength;
    public Vector3 size;
   ...
   [SerializeField] AttatkData attackData;
    ...
    void AttackOnShape(float r,int damage )
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            if (traget != null) { traget.TakeDamage(damage, 2f, transform, currentShakeStrength); }
        }
    ...
    void AttackOnBox(AttackID iD)
    {
       Attackinfo attackInfo = attackData.info.Find(x=>x.attackID == iD);
        ...
        currentShakeStrength= attackInfo.currentShakeStrength;
        ...
        {
            IDamageable traget = colliders[i].GetComponent<IDamageable>();
            if (traget != null) { traget.TakeDamage(currentDamageRate, currentknockbackrate, transform,currentShakeStrength); }
        }
    }
