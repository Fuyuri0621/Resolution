...
public class BossControl : MonoBehaviour,IDamageable
{
   ...
   public GameObject BossinfoPanel;
    ...
    private void HandleDeath()
    {
        ...
        GameManager.Instance.RemovetBattleEmemies(gameObject);
   ...
   [SerializeField] List<string> attacklist;
    ...
    string RandomRangeAttack()
    {
        ...
        return rangeAttacklist[UnityEngine.Random.Range(0, rangeAttacklist.Count)];
    ...
    public void TakeDamage(int damage,int stun,float knockback,AllPlayerCharacter attacker, ShakeStrength shakeStrength = ShakeStrength.LIGHT)
    {
        ...
        float hitstopduration = GameManager.Instance.hitstopduration;
    ...
    public void NextStage()
    {
        ...
        attacklist.Add("BossAttackC");
        nav.allowMoving = false;
        nextStage=true;
        GameManager.Instance.PlayingAni();
        Cursor.visible = false;

        nextStageCam.SetActive(true);
        GetComponent<BossSpecialCol>().enabled = true;
        stats.state = EnemyAiState.idle;

        StartCoroutine(ReturnBattle(40));
        
    }

    IEnumerator ReturnBattle(float duration)
    {
        yield return new WaitForSeconds(duration);
    ...
    public void ThrowBomb()
    {
       GameObject b = Instantiate(bomb);
        b.GetComponent<BossProjectile>().SetPoint(transform.position,GetNewWanderPosition()+Vector3.up*10,target.transform.position);
    }

    public enum BombPattern { Random, Line, Circle }
    ...
    void IDamageable.TakeDamage(int damage, float knockback, Transform attackform, ShakeStrength shakeStrength)
    {
        return;
    }
}
