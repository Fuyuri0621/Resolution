...
public class InteractableObject : MonoBehaviour,IInteractable
{
    ...
    void Start()
    {
        ...
        if(dooranimator != null) dooranimator.Play("CabinetDoorClose");
    }
    ...
    public bool Interact(Interactor interactor)
    {
        if (GameManager.Instance.IsControlling&&haveItem)
        {
            if (_prompt == "healitem")
            {
                NoticeManager.Instance.SendNotice(GameManager.Instance.GetBackpackItemById(7).name, 2f, 0);
                interactor.GetComponent<Healcontrol>().AddHealamout(1);
