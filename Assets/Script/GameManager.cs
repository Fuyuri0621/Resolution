...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] ALLSecne currentScene;
    public Light lights;
    ...
    private void Awake()
    {
        ...
        PlayerState = canvas.transform.Find("PlayerState").GetComponent<Animator>();
        Time.timeScale = 1.0f;
        _instance = this;
    ...
    void Start()
    {
        ...
        AudioManager.Instance.PlayBGM(nomalBGM);

        if (TempSave.selectedData != null)
        {
            datacontainer = TempSave.selectedData;

            Debug.Log("載入資料成功：" + datacontainer.currentScene);
    ...
    private void Loadsavedata()
    {
        ...
        healAmout=datacontainer.healAmout;
          MissionManager.Instance.SwitchMission(datacontainer.lastmissionid);
    }

    public void Savesavedata()
    {
        TempSave.selectedData.currentScene = currentScene;
        TempSave.selectedData.lastCheckpoint = GetCtrolingCharacter().transform.position;
        TempSave.selectedData.lastmissionid = MissionManager.Instance.Getnowmissionid();
        TempSave.selectedData.healAmout = healAmout;

        TempSave.selectedData.savedate = DateTime.Now.ToString();
    }
    ...
    {
        Instantiate(missionpanel, canvas.transform);
    }
    public void OpenMapPanel()
    {
        Instantiate(mappanel, canvas.transform);
    }
    public void OpenInfoPanel()
    {
        Instantiate(infopanel, canvas.transform);
    }
    ...
    public void OnOpenMission(InputValue value)
    {
        if (IsGameRunning && IsControlling)
        {
            OpenMissionPanel();
    ...
    [SerializeField] AudioClip nomalBGM;
   ...
   public float hitstopduration = 0.14f;
    ...
    {
        isBattling = true;
        AudioManager.Instance.SwitchBGM(battleBGM);
    ...
    {
        isBattling = false;
        AudioManager.Instance.SwitchBGM(nomalBGM);
        if (ChinLicontrol.gameObject.activeInHierarchy)
...
public bool IsthisCtrolingCharacter(AllPlayerCharacter c)
    ...
    public void ActiveChinLi(Transform t) { ActiveCharacter(AllPlayerCharacter.ChinLi, t); }
      public void ActiveTanTong(Transform t) { ActiveCharacter(AllPlayerCharacter.TanTong, t); }
   ...
   public IEnumerator PlayerHitStop(AllPlayerCharacter player,float duration)
    {
        IHitStopHandler hitStop = GetPlayerGameObject(player).GetComponent<IHitStopHandler>();

        hitStop.OnHitStop(true);
        yield return new WaitForSeconds(duration);

        hitStop.OnHitStop(false);
    }
}
