...
public class HackPC : MonoBehaviour,IInteractable
{
    ...
    [SerializeField]  List<GameObject> HackStage;
    ...
    public bool Interact(Interactor interactor)
    {
        if (allowInteract)
        {
            NoticeManager.Instance.SendNotice("使用WASD移動來吃到黑點", 3f,0f);
            allowInteract = false;
            allowUI.SetActive(false);
            GameManager.Instance.SetIsControlling(false);
            Debug.Log("hacking");

            int rnd =UnityEngine.Random.Range(0,HackStage.Count);

            stage = Instantiate(HackStage[rnd]);
